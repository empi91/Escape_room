# text.py
# Interface text

intro = "Witaj {player_name}, dobrze, że się obudziłeś! \n\
Znajdujesz się pośrodku dużego pokoju, nie wiesz gdzie jesteś, nie wiesz co się stało. \n\
Chcesz tylko wrócić do domu.. \n\n\
Pod sufitem pali się lampa i widzisz, że w pokoju znajduje się: \n\
{location0}, {location1}, {location2} oraz {location3} \n\
W tej chwili stoisz przed drzwiami."

menu = "Wybierz co chcesz zrobić: "

interface_help = "Masz do wyboru następujące opcje: \n\
Obejrzyj miejsce, przy który stoisz (/sprawdź) | \
Przejdź w lewo (/lewo) lub prawo (/prawo) | \
Weź przedmiot (/weź) | \n\
Użyj przedmiotu z plecaka (/użyj) | \
Odłóż przedmiot (/odłóż) | \
Zajrzyj do plecaka (/plecak) | \
Wyświetl dostępne komendy (/pomoc) | \
Wyjdź z gry (/zakończ) \n"

# Locations descriptions

door_desc = "Stoisz przed starymi, solidnymi drzwiami. Nie mają okna ani klamki, \n\
a jedyne co widzisz to starodawny, mosiężny zamek z dużą dziurką na klucz"

table_desc = "Przed tobą stoi duży drewniany stół, na którym leżą: "

cupboard_desc = "CO prawda nie za duża i niezbyt nowa, ale dalej solidna, drewniana szafka ma jedną szufladę. \n\
Oczywiście zamkniętą, ale na szczęscie jest również zamek, który wygląda na sprawny."

window_desc = "Podchodzisz do otwartego, lecz zakratowanego okna. Już na pierwszy rzut oka widzisz,\n\
że tędy na pewno nie uciekniesz. Ale za to w rogu parapetu dostrzegasz kawałek wystającej kartki. \n\
Co to może być?"

# Text for script scenarios

scenario_1_text = "Wkładasz klucz do zamka szuflady i okazuje się, że nawet pasuje! Przekręcasz go, otwierasz szufladę \n\
i Twoim oczom ukazuje się skrzyneczka zamknięta kłódką z miejscem na wpisanie trzycyfrowego kodu."

scenario_2_text = ""

scenario_3_text = "Z obawą wkładasz stary klucz do zamka i próbujesz przekręcić. \n\
Zgrzyta, brzęczy, ale w końcu udaję się wykonać pół obortu, potem następne, aż w końcu słyszysz, \n\
jak zapadki się przesuwają i drzwi stają otworem. \n\
Jesteś wolny !! \n\n Tylko czy aby na pewno?!?!"
